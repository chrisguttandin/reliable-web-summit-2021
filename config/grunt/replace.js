...
const replaceHashInMatch = (grunt, match, prefix, index) => {
    const filename = grunt.file.expand({ cwd: 'build/reliable-web-summit-2021/scripts' }, `${prefix}.*.js`)[0];

    if (filename === undefined) {
        return match;
    }

    const hash = `sha384-${computeHashOfFile(`build/reliable-web-summit-2021/scripts/${filename}`, 'sha384', 'base64')}`;
...
module.exports = (grunt) => {
    return {
                ...
                patterns: [
                    {
                        ...
                        replacement: () => {
                            const html = fs.readFileSync('build/reliable-web-summit-2021/index.html', 'utf8'); // eslint-disable-line node/no-sync
        ...
        'manifest': {
            files: {
                './': ['build/reliable-web-summit-2021/ngsw.json']
            },
            options: {
                patterns: [
                    {
                    ...
                    },
                    {
                        ...
                        match: /"\/reliable-web-summit-2021(?<filename>\/scripts\/(?:\d+|main|scripts)(?:-es(?:2015|5))?.[\da-z]+.js)":\s*"[\da-z]+"/g,
                        replacement: (_, filename) => {
                            return `"/reliable-web-summit-2021${filename}": "${computeHashOfFile(
                    ...
                    },
                    {
                        ...
                        match: /"\/reliable-web-summit-2021(?<filename>\/styles\/styles\.[\da-z]*\.css)":\s*"[\da-z]+"/g,
                        replacement: (_, filename) => {
                            return `"/reliable-web-summit-2021${filename}": "${computeHashOfFile(
                                `build/reliable-web-summit-2021${filename}`,
                                'sha1',
                    ...
                    },
                    {
                        ...
                        replacement: (_, filename) => {
                            return `"/reliable-web-summit-2021/${filename}.html": "${computeHashOfFile(
                                `build/reliable-web-summit-2021/${filename}.html`,
                                'sha1',
                                'hex'
                            )}"`;
                        }
                    ...
                    },
                    {
                        ...
                        replacement: (_, filename, media) => {
                            const hash = `sha384-${computeHashOfFile(
                                `build/reliable-web-summit-2021/styles/${filename}`,
                                'sha384',
                                'base64'
                            )}`;
